 ...
 * 格内的蜂巢（bee_nest）打上已交互标记，渲染时只高亮已点过（出过蜜蜂）的蜂巢。
 * 蜂箱（beehive）代表该位置本次没刷蜜蜂，忽略。多人 Safari 队友交互的
 * 蜜蜂也会触发实体加载，标记同样生效。
 *
 * 不做扫图：不需要预先找出未交互蜂巢，只要记录 Bee 出现位置附近的蜂巢即可。
 */
public final class SafariBeeNestHighlight {
    ...
    public static void init() {
        ClientEntityEvents.ENTITY_LOAD.register((entity, level) -> {
            ...
            if (entity.getType() == EntityType.BEE && inSafari) {
    ...
    /** Bee 实体出现 → 周围 2 格内的蜂巢视为已交互/已点过（蜜蜂消失不清，蜂巢是一次性的） */
    ...
    private static void render(WorldRenderContext context) {
        ...
        if (interactedNests.isEmpty()) return;

        int rgb = config().beeNestHighlightColor.getEffectiveColourRGB();
        float red = (rgb >> 16 & 0xFF) / 255.0f;
        float green = (rgb >> 8 & 0xFF) / 255.0f;
        float blue = (rgb & 0xFF) / 255.0f;
        // 实心盒子比蜂巢方块外扩一点,不与方块表面贴合
        double expand = 0.01;
        for (BlockPos pos : interactedNests) {
