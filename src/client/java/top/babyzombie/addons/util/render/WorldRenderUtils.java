...
public final class WorldRenderUtils {
    ...
    public static void drawFilledCircle(WorldRenderContext context,
        ...
        if (trianglesBuf == null) {
            trianglesBuf = new BufferBuilder(ALLOCATOR, pipeline.getVertexFormatMode(), pipeline.getVertexFormat());
        }
        var pose = applyCameraTransform(context);
    ...
    private static void uploadAndDrawFilled(RenderPipeline pipeline, BufferBuilder buf) {
        ...
        GpuBuffer vertices = uploadVertices(drawParameters, format, builtBuffer, 0);
    ...
    private static void uploadAndDrawLines(RenderPipeline pipeline, BufferBuilder buf) {
        ...
        GpuBuffer vertices = uploadVertices(drawParameters, format, builtBuffer, 1);
        draw(pipeline, builtBuffer, drawParameters, vertices, format);
    ...
    private static void uploadAndDrawTriangles(RenderPipeline pipeline, BufferBuilder buf) {
        ...
        GpuBuffer vertices = uploadVertices(drawParameters, format, builtBuffer, 2);
        draw(pipeline, builtBuffer, drawParameters, vertices, format);
    ...
    /**
     * Upload vertex data to the appropriate ring buffer.
     * @param kind 0 = filled (QUADS), 1 = lines, 2 = triangles
     */
    private static GpuBuffer uploadVertices(MeshData.DrawState drawParameters, VertexFormat format,
                                             MeshData builtBuffer, int kind) {
